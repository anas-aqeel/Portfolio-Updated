...
import type { DiaryType, PostType } from '@/types'
import { getSingleDiary } from '@/lib/sanity.query'
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const post: PostType = await getSingleDiary(slug)
  if (!post) {
    notFound()
  }
  ...
  return {
    title: `${post.title}`,
    metadataBase: new URL(`https://ai-developer.vercel.app/diary/${post.slug}`),
    description: post.description,
    publisher: post.author.name,
    keywords: post.tags,
    alternates: {
      canonical: `https://ai-developer.vercel.app/diary/${post.slug}`,
    },
    openGraph: {
      images:
        // urlFor(post.coverImage?.image)?.width(1200)?.height(630)?.url() ||
        fallbackImage,
      url: `https://ai-developer.vercel.app/diary/${post.slug}`,
      title: post.title,
      description: post.description,
      type: 'article',
...
export default async function Post({ params }: Props) {
  const slug = params.post
  const post: DiaryType = await getSingleDiary(slug)
  if (!post) {
    notFound()
  }
